...
INCLUDES := -I/home/hardav/local/usr/include -I$(CUDA_PATH)/include -I$(CUDA_PATH)/targets/ppc64le-linux/include
LIBRARIES := -L/home/hardav/local/usr/lib64 -L$(CUDA_PATH)/lib64 -L$(CUDA_PATH)/targets/ppc64le-linux/lib
...
$(BUILDDIR)/melgan: $(OBJ) $(OBJDIR)/main.o
	$(EXEC) $(NVCC) $(ALL_LDFLAGS) $(GENCODE_FLAGS) -o $@ $+ $(INCLUDES) $(LIBRARIES)

$(BUILDDIR)/test_melgan: $(OBJ) $(TESTOBJ)
	$(EXEC) $(NVCC) $(ALL_LDFLAGS) $(GENCODE_FLAGS) -o $@ $+ $(INCLUDES) $(LIBRARIES)

$(OBJDIR)/%.o: $(SRCDIR)/%.cpp $(INC)
	$(EXEC) $(HOST_COMPILER) $(INCLUDES) $(CCFLAGS) $(EXTRA_CCFLAGS) -o $@ -c $<

$(OBJDIR)/%.o: $(SRCDIR)/%.cu $(INC)
	$(EXEC) $(NVCC) $(INCLUDES) $(ALL_CCFLAGS) $(GENCODE_FLAGS) -o $@ -c $<

$(TESTOBJDIR)/%.o: $(TESTDIR)/%.cpp $(INC)
	$(EXEC) $(HOST_COMPILER) $(INCLUDES) $(CCFLAGS) $(EXTRA_CCFLAGS) -I src/ -o $@ -c $<

run: build
	$(EXEC) ./$(BUILDDIR)/melgan

clean:
	rm -rf $(OBJDIR)/*.o
	rm -rf $(TESTOBJDIR)/*.o
	rm -rf $(BUILDDIR)/melgan
	rm -rf $(BUILDDIR)/test_melgan
